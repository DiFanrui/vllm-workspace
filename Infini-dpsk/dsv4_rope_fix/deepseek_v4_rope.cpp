...
void create_gpu_ropes_if_needed(DeepseekV4RoPE::Backend backend,
    ...
    // The main rope is used for q / kv_sw / output on EVERY layer (including
    // compressed layers), so it must always be created.
    const size_t main_max_seq_len = rope_cache_max_seq_len(model_config, config_json, false);
...
DeepseekV4RoPE::DeepseekV4RoPE(std::shared_ptr<infinilm::config::ModelConfig> model_config,
    ...
    // Output inverse RoPE always uses the main rope: the reference applies
    // rope_theta to q / kv_sw / output, and only the compressed KV uses
    // compress_rope_theta. (YaRN scaling for compressed KV lives in compress_rope_).
    params_.use_yarn = false;
    params_.rope_theta = rope_theta;
...
infinicore::Tensor DeepseekV4RoPE::forward_gpu_(const infinicore::Tensor &x,
                                                ...
                                                const infinicore::Tensor &device_positions,
                                                const std::shared_ptr<infinicore::nn::RoPE> &rope) const {
    ...
    if (!disable_inplace_partial_rope()) {
        ...
        rope->forward(rope_slice, pos_tensor, true);
        return x;
    }

    auto nope = x->narrow({{3, 0, pass_dim}});
    auto rope_slice = x->narrow({{3, pass_dim, rope_dim}})->contiguous();
    auto rotated = rope->forward(rope_slice, pos_tensor, true);
...
infinicore::Tensor DeepseekV4RoPE::forward(const infinicore::Tensor &x,
    ...
    return forward_gpu_(x, positions, device_positions, main_rope_);
}

infinicore::Tensor DeepseekV4RoPE::forward_compress(const infinicore::Tensor &x,
                                                    const std::vector<int64_t> &positions,
                                                    const infinicore::Tensor &device_positions) const {
    return forward_gpu_(x, positions, device_positions, compress_rope_);
}
