...
__global__ void compressed_decode_kernel(T *__restrict__ y,
    ...
    // block b covers tokens [b*m, b*m+m-1]; visible iff block_end = b*m+(m-1) < q_pos  =>  b < q_pos/m
    const size_t visible_blocks = static_cast<size_t>(max(static_cast<int64_t>(0), q_pos / static_cast<int64_t>(compress_ratio)));
    ...
    for (size_t pair = tid; pair < half; pair += blockDim.x) {
        // split-half layout matching reference _rotate_half (dim i paired with dim half+i)
        const size_t first = pass_dim + pair;
        const size_t second = pass_dim + half + pair;
        float acc_first = 0.0f;
        ...
        for (size_t slot = 0; slot < compressed_keys; ++slot) {
            ...
            if (valid) {
                ...
                const size_t block = static_cast<size_t>(selected_block);
                const size_t comp_base = comp_batch_base + block * head_dim;
                acc_first += prob * to_float<T>(kv_comp[comp_base + first]);
                acc_second += prob * to_float<T>(kv_comp[comp_base + second]);
            }
        }
        for (size_t j = 0; j < key_len; ++j) {
            const float prob = logits[compressed_keys + j];
            const size_t k_offset = k_base + j * num_kv_heads * head_dim;
            acc_first += prob * to_float<T>(k[k_offset + first]);
            acc_second += prob * to_float<T>(k[k_offset + second]);
        }
        ...
        y[q_base + first] = from_float<T>(acc_first * c - acc_second * s);
        y[q_base + second] = from_float<T>(acc_second * c + acc_first * s);
    }
}
