...
DeepseekV4TopK::DeepseekV4TopK(std::shared_ptr<infinilm::config::ModelConfig> model_config,
    ...
    INFINICORE_NN_PARAMETER_INIT(weight, ({num_experts_, hidden_size_}, infinicore::DataType::F32, device));
...
DeepseekV4TopK::forward(const infinicore::Tensor &hidden_states) const {
    ...
    auto gate_logits = hidden_states->dtype() == infinicore::DataType::BF16
                         && weight_->dtype() == infinicore::DataType::F32
                         ? infinicore::op::dsv4_linear_bf16_fp32(hidden_states, weight_)
                         : infinicore::op::linear(hidden_states, weight_, std::nullopt, 1.0f);

    auto [topk_weights, topk_indices] = infinicore::op::deepseek_v4_topk_router(
        gate_logits, num_experts_per_tok_, scoring_func_ != "softmax" || norm_topk_prob_, router_bias);
...
DeepseekV4HashTopK::forward(const infinicore::Tensor &hidden_states,
    ...
    // The reference computes routing logits in FP32. The fused hash router
    // currently performs its BF16 linear internally, so use the explicit
    // FP32-logit path for BF16 checkpoints.
    ...
    auto [topk_weights, topk_indices] = infinicore::op::deepseek_v4_hash_router(
        ...
        input_ids_view,
        tid2eid_,
        scoring_func_ != "softmax" || norm_topk_prob_);
    if (routed_scaling_ != 1.0f) {
        topk_weights = infinicore::op::mul_scalar(topk_weights, routed_scaling_);
    }
    return {topk_weights, topk_indices};
