    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        for weight_name, weight_param in weight_dict.items():
            ...
            if weight_name == "awq_scale":
                # 逐输入通道 scale（[1, in]）：共享、不按输出 shard。
                # 不设 output_dim，使 QKVParallel/MergedColumnParallel weight_loader 的
                # shard 分支（shard_id="q"/"k"/"v"、0/1）走"整张拷贝"else 分支 ——
                # 各 shard 幂等写入同一个 [1, in]；Row/Column 路径照常按 input_dim=1 载入。
                set_weight_attrs(param, {"input_dim": 1})
            else:
...
class AscendFusedMoEMethod(FusedMoEMethodBase):
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        # Schemes that store whole fused tensors (e.g. hand-written AWQ MoE) load
        # through the generic 2-arg `weight_loader(param, loaded_weight)` branch
        # of the model's load_weights.  FusedMoE.weight_loader has a 6-arg
        # signature and would break that branch, so drop it for these schemes.
        weight_attrs = extra_weight_attrs
        if getattr(self.quant_method, "load_whole_tensor", False):
            weight_attrs = {
                **extra_weight_attrs,
        ...
        for param_key, param_value in weight_param.items():
            ...
            set_weight_attrs(param, weight_attrs)
